...
public class PaneProgPane extends javax.swing.JPanel
    ...
    public PaneProgPane(PaneContainer parent, String name, Element pane, CvTableModel cvModel, VariableTableModel varModel, Element modelElem, RosterEntry pRosterEntry, boolean isProgPane) {
        ...
        JPanel p = builder.build(rootPane, 
                createLayoutBuilder(rootPane));

        this.isCvTablePane = this._cvTable = builder.isIsCvTablePane();
        this.fnMapList = builder.getFnMapList();
        this.fnMapListESU = builder.getFnMapListESU();
        this.panelList = builder.getPanelList();
        this.cvList = builder.getCvList();
        this.varList = builder.getVarList();
...
//      Only add change buttons to CV tables
    ...
    protected ControlXMLReader createControlBuilder(Element pane, Element modelElem) {
        return new ControlXMLReader(pane, _cvModel, _varModel, rosterEntry, modelElem);
    }

    // for testing
    protected LayoutBuilder createLayoutBuilder(JPanel pane) {
        return new GridBagLayoutBuilder.RootBuilder(pane);
    }
