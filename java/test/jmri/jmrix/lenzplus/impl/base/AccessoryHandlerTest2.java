...
public class AccessoryHandlerTest2 implements CommandService, TrafficController {
    ...
    List<XNetPlusMessage> sent = new ArrayList<>();
    XNetPlusMessage m;
    AccessoryHandler h;
    CommandState s;
    ...
    public void setUp() {
        m = XNetPlusMessage.create(
                ...
                true, false, true));
        s = queue.send(null, m, null);
        h = (AccessoryHandler)s.getHandler();
    ...
    public void testAcceptsOK() throws Exception {
        assertTrue(h.acceptsReply(m, ok));
    }
    ...
    public void testAcceptsJustSingleOK() throws Exception {
        ...
        assertFalse(h.acceptsReply(m, ok));
    }
    ...
    @Test
    public void testFinishedWithRecheck() throws Exception {
    ...
    protected void checkSentCommand() {
        ...
        assertNotSame(s, st);
        assertTrue(
                !lastSentState.getMessage().getCommandedOutputState() &&
    ...
    private void testFinishedWithReply(XNetPlusReply r) throws Exception {
        ...
        CommandState s2;
        synchronized (queue) {
            queue.replyFinished(out);
            f = queue.getFutureCommand(s2 = lastSentState);
            assertNotNull(f);
